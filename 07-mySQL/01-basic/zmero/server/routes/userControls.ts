...
export function register(req, res) {
  try {
    ...
    const sql = `INSERT INTO cats.owners(owner_id,name,city,password) VALUES (${uid()},'${name}','${city}', ${password});`;
...
export function addCat(req, res) {
  try {
    ...
    if (!name || !age) throw new Error("invalid fields")
    const sql = `INSERT INTO cat(cat_id,name,age) VALUES("${Date.now()}","${name}","${age}");`
    con.query(sql, (err, result) => {
      try {
        if (err) throw err;
        console.log("1 record inserted");
...
export function getCatUnder2(req, res) {
  const sql = `Select * from cat where cat.age<=2;`
  try {
    con.query(sql, (err, result) => {
      try {
        if (err) throw err;
        console.log("cat get success");
        res.send({ results: result });
      } catch (err) {
        console.log(err.message);
        res.send({ error: err.message });
