    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchAllRestaurants.fulfilled, (state, action) => {
                state.status = 'idle';
                const arrayHashmap = action.payload.reduce((obj: any, item: any) => {
                    if (obj[item.id]) {
                        obj[item.id].photos.push(item.url)
                        obj[item.id].subCategory.push(item.category_name)
                    } else {
                        obj[item.id] = {
                            "id": item.id, "name": item.name, image: item.image, region: item.region,
                            stars: item.stars, city: item.city, category: item.category,
                            photos: [item.url], open: item.open,
                            close: item.close,
                ...
                state.arrOfResteruants = Object.values(arrayHashmap);
            })
            ...
            .addCase(fetchFamousRestaurants.fulfilled, (state, action) => {
                ...
                state.arrOfFamousResteruants = Object.values(arrayHashmap);
            })
