...
export const fetchAccident=createAsyncThunk(
    'accident/fetchAccidentById',
    async(userId:any)=>{
        try{
            const response=await axios.post('http://localhost:3001/accidents/',{userId})
             return response.data;
        }
        catch (err: any) {
            console.log(err.message)
...
export const accidentReducer = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(fetchAccident.pending, (state) => {
                state.status = 'loading';
            })
            .addCase(fetchAccident.fulfilled, (state, action) => {
                state.status = 'idle';
                console.log(action);
